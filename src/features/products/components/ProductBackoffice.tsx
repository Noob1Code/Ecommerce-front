  ...
  return (
    <RoleGuard
      ...
      fallback={
        <div className="mx-auto max-w-xl px-4 py-16 text-center">
          <Card className="bg-red-50 border border-red-200 rounded-xl p-6 sm:p-8 shadow-sm">
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 sm:py-12">
        ...
        {/* Topbar do Painel Administrativo */}
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between border-b border-gray-200 pb-5 mb-6">
          <div>
          ...
          {/* SOLUÇÃO TOPBAR MOBILE: Substituído o flex-wrap desalinhado por um grid em coluna única que vira flex no desktop */}
        ...
        {abaAtiva === 'atributos' ? (
          ...
          <>
            {exibirFormCriacao && (
              <div className="mb-8"><CreateProductForm onClose={() => setExibirFormCriacao(false)} /></div>
            )}

            {/* Barra de Pesquisa e Combobox de Status */}
            <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center max-w-3xl w-full">
              ...
              <div className="w-full sm:w-1/2">
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-300 bg-white focus:outline-none font-semibold text-gray-700 shadow-2xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                >
            ...
            <div className="space-y-6">
              {produtosFiltrados.map((product) => {
                ...
                return (
                  <Card key={product.id} className={`p-4 sm:p-5 border rounded-xl bg-white shadow-xs transition-colors ${!product.isActive ? 'border-red-200 bg-red-50/10' : 'border-gray-200'}`}>

                    {/* SOLUÇÃO GRID CARD: Força empilhamento em bloco flexível vertical no celular, abrindo grid a partir de md: */}
                    <div className="flex flex-col gap-4 border-b border-gray-100 pb-5 mb-4 md:grid md:grid-cols-3 md:items-end">
                      <div className="w-full space-y-1.5">
                        <div>
                          ...
                          <Input
                            ...
                            className="text-sm font-bold text-gray-900 rounded-xl"
                          />
                        </div>
                      ...
                      {/* SOLUÇÃO BOTÕES CARD: Modificado de flex-row para flex-col no celular. Agora os botões ocupam largura cheia e não quebram o layout */}
                      <div className="flex flex-col sm:flex-row gap-2 w-full md:justify-end">
                        <button
                          ...
                          className="w-full sm:w-auto text-center text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100 rounded-xl px-4 py-3 sm:py-2.5 hover:bg-blue-600 hover:text-white transition-all active:scale-95 shadow-3xs"
                        >
                        ...
                        <button
                          ...
                          className={`w-full sm:w-auto text-center text-xs font-bold rounded-xl px-4 py-3 sm:py-2.5 border transition-all active:scale-95 shadow-3xs ${product.isActive ? 'text-red-600 bg-red-50 border-red-100 hover:bg-red-600 hover:text-white' : 'text-green-600 bg-green-50 border-green-100 hover:bg-green-600 hover:text-white'}`}
                        >
                    ...
                    {/* Eixos de Atributos Vinculados */}
                    {product.isActive && (
                      <div className="mb-4 p-3 bg-gray-50 border border-gray-200 rounded-xl space-y-2 shadow-3xs">
                    ...
                    {/* Sub-tabela de Variações Físicas de SKU */}
                    <div className="w-full overflow-x-auto rounded-xl border border-gray-100">
                      <table className="min-w-[650px] w-full divide-y divide-gray-100 text-xs text-left">
                        <thead className="bg-gray-50 text-gray-500 font-bold uppercase tracking-wider text-[10px]">
                          <tr>
                            <th className="px-4 py-3 w-[22%]">SKU</th>
                            <th className="px-4 py-3 w-[25%]">Opções</th>
                            <th className="px-4 py-3 w-[13%]">Preço Base</th>
                            <th className="px-4 py-3 w-[15%]">Preço Novo</th>
                            <th className="px-4 py-3 text-center w-[13%]">Estoque</th>
                            <th className="px-4 py-3 text-right pr-6 w-[12%]">Ações</th>
                          </tr>
                        </thead>
                            ...
                            return (
                              <Fragment key={sku.id}>
                                <tr className="hover:bg-gray-50/40 transition-colors">
                                  <td className="px-4 py-3 font-mono font-bold text-gray-900 truncate max-w-[130px]" title={sku.skuCode}>{sku.skuCode}</td>
                                  <td className="px-4 py-3 text-gray-500 font-medium truncate max-w-[160px]" title={sku.options?.map((o) => `${o.attributeName}: ${o.value}`).join(' | ')}>{sku.options?.map((o) => `${o.attributeName}: ${o.value}`).join(' | ')}</td>
                                  ...
                                  <td className="px-4 py-3">
                                    <div className="flex items-center justify-center gap-1.5">
                                      <button type="button" onClick={() => handleDecrementarEstoque(sku.id, sku.stock)} className="h-6 w-6 border border-gray-300 rounded-md bg-gray-50 text-gray-600 font-bold select-none shadow-3xs active:scale-90 transition-transform">-</button>
                                      <input
                                        type="number"
                                        value={estEfetivo}
                                      ...
                                      />
                                      <button type="button" onClick={() => handleIncrementarEstoque(sku.id, sku.stock)} className="h-6 w-6 border border-gray-300 rounded-md bg-gray-50 text-gray-600 font-bold select-none shadow-3xs active:scale-90 transition-transform">+</button>
                                    </div>
                                  </td>
                                  <td className="px-4 py-3 text-right pr-6">
                                    <div className="flex justify-end gap-3">
                                      ...
                                      <button type="button" onClick={() => handleExclusaoFisicaSku(sku.id, sku.skuCode)} className="text-xs font-bold text-red-600 hover:text-red-800 transition-colors active:scale-95">Excluir</button>
                                    </div>
                                  </td>
                                </tr>

                                {/* Painel Expandido Inline para Edição de Opções */}
                                    ...
                                    <td colSpan={6} className="px-4 py-4 border-t border-b border-blue-100">
                                      <div className="space-y-4 max-w-xl animate-in fade-in duration-150">
                                                ...
                                                <div key={idx} className="relative group rounded-lg overflow-hidden border border-gray-200 aspect-square bg-white">
                                                  ...
                                                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                                                    <button
                                                      ...
                                                      className="bg-red-600 text-white rounded px-1.5 py-0.5 text-[9px] font-bold hover:bg-red-700 disabled:opacity-50"
                                                    >
                                                      Excluir
                                                    </button>
                                                  </div>
                                        ...
                                        <div className="flex gap-2.5 justify-end pt-2">
                                          <Button type="button" variant="secondary" onClick={handleCancelarEdicaoSku} className="text-xs py-2 px-4 rounded-xl">
                                            Cancelar
                                          </Button>
                                          <Button
                                            type="button"
                                            ...
                                            className="text-xs py-2 px-4 bg-blue-600 text-white hover:bg-blue-700 rounded-xl border-none font-bold shadow-sm"
                                          >
                                            Salvar Alterações
                                          </Button>
                                        </div>
