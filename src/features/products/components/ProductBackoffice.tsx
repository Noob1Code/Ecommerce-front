        ...
        <div className="sm:flex sm:items-center sm:justify-between border-b border-gray-200 pb-5 mb-6 gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-500">
              Gerencie produtos pai, configure estoques/preços de variações e defina atributos estruturais.
            </p>
          </div>
          <div className="mt-4 sm:mt-0 flex flex-wrap items-center gap-3">
            ...
            {abaAtiva === 'produtos' && (
              ...
              >
                {exibirFormCriacao ? 'Fechar Cadastro' : 'Cadastrar Produto Container'}
              </Button>
            )}
            ...
            <Button
              ...
              onClick={() => setAbaAtiva((prev) => prev === 'produtos' ? 'atributos' : 'produtos')}
              className={`px-4 py-3 text-sm font-bold uppercase tracking-wider shadow-md transition-all ${abaAtiva === 'atributos' ? 'bg-purple-600 border-purple-600 text-white hover:bg-purple-700' : 'text-purple-700 border-purple-200 bg-purple-50 hover:bg-purple-100'
                }`}
            >
              {abaAtiva === 'produtos' ? 'Gerenciar Atributos (Cor/Tam)' : 'Voltar para Produtos'}
            </Button>
            ...
            {abaAtiva === 'produtos' && (
              ...
              >
                {estaEnviando ? 'Salvando alterações...' : `Salvar Alterações em Lote (${contagemModificados})`}
              </Button>
            )}
          </div>
        </div>

        {/* NAVEGAÇÃO ENTRE ABAS DO CRUD COMPLETO */}
        {abaAtiva === 'atributos' ? (
          <AttributeManager />
        ) : (
          <>
              ...
              {produtosFiltrados.map((product) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-gray-100 pb-5 mb-5 items-start">
                      ...
                      <div className="md:col-span-1 flex justify-end items-center pt-5 md:pt-4 gap-2">
                        ...
                        <RoleGuard allowedRoles={['ROLE_ADMIN', 'ROLE_ESTOQUE']}>
                          ...
                          >
                            {product.isActive ? 'Inativar Produto' : 'Reativar Produto'}
                          </button>
                        </RoleGuard>
                      </div>
                    </div>

                    {/* Formulário Intermediário de Variação de SKU */}
                    ...
                    {/* Sub-tabela de Variações de SKU (ProdutoVariacaoController) */}
