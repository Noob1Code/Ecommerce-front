...
export const CustomerProfile = () => {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-4 sm:py-10 w-full">
      <div className="mb-6 border-b border-gray-200 pb-5 text-center sm:text-left">
        <button
          type="button"
          onClick={handleBackToCatalog}
          className="text-xs font-bold text-blue-600 hover:text-blue-800 uppercase tracking-wider flex items-center justify-center sm:justify-start space-x-1 active:scale-95 transition-transform mb-3 mx-auto sm:mx-0 py-1"
        >
          <span>&larr; Voltar para o Catálogo</span>
        </button>
        <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-gray-900">
          Meus Dados Cadastrais
        </h1>
        <p className="mt-1.5 text-xs sm:text-sm text-gray-500 font-medium px-1">
          {ehCliente
      ...
      <Card className="p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm w-full">
        <form onSubmit={handleSalvarAlteracoes} className="space-y-4 sm:space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input id="nome" type="text" value={nome} onChange={(e) => setNome(e.target.value)} required placeholder="Seu name completo" disabled={estaCarregando} className="rounded-xl" />
            </div>
            <div>
          ...
          {ehCliente ? (
            ...
            <div className="grid grid-cols-1 gap-4 animate-in fade-in duration-200">
              <div className="w-full sm:max-w-md">
                <label htmlFor="matricula" className="block text-xs font-bold text-gray-500 uppercase mb-1 px-0.5">
                  Matrícula Funcional *
                </label>
          ...
          <div className="grid grid-cols-1 gap-4 border-t border-gray-100 pt-4">
            <div className="w-full sm:max-w-md">
              <label htmlFor="senha" className="block text-xs font-bold text-gray-500 uppercase mb-0.5 px-0.5">
                Alterar Senha de Acesso
              </label>
              <p className="text-[11px] text-gray-400 mb-1.5 normal-case px-0.5">
